...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for facility in Facility.objects.filter(cluster=True):
            sms_message = CLUSTER_DAILY_TPL['sms'].format(uuid=facility.identity, link=facility.qualtrics_link())
            email_message = CLUSTER_DAILY_TPL['email'].format(uuid=facility.identity, link=facility.qualtrics_link(), facility_name=facility.name)
            if sms_enabled:
